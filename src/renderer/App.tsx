...
const data = [
  ...
  'D DOS攻击就是很多人同时访问了一个服务器，这就是为什么浏览器会限制同时发起TCP链接的个数',
  '应用层 传输层 网络层 链路层 物理层',
...
const Hello = () => {
  ...
  function createTime(current, time) {
    timerRef.current = setTimeout(() => {
      ...
      createTime(nextCurrent, getTime(knowledgePointArr[nextCurrent].length));
  ...
  useEffect(() => {
    ...
    ref.current = 0;
    clearTimeout(timerRef.current);
    createTime(0, getTime(knowledgePointArr[0].length));
  }, [knowledgePointArr.length]);

  const addValue = () => {
    if (value.length >= 425) {
      message.warning('知识点长度不能超过425个字符');
      return;
    }
    if (!value) return;
    setKnowledgePointArr([value, ...knowledgePointArr]);
    setValue('');
  };
